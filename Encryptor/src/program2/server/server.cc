...
void Server::acceptConnection() {
  ...
  server = accept(connection_,
                  reinterpret_cast<struct sockaddr*>(&server_address_), &size);
  if (server < 0) {
...
void Server::dataProcessing() {
  if (connection_check_ == true) {
    if (buffer == -9) {
      ...
      std::cout << "ERROR: received number < 10 or not a multiple of 32"
                << std::endl;
    }
  }
}