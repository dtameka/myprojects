...
void Client::collectData() {
  // При первом запуске программы, помогает потокам начать работать в нужном
  // порядке(костыль)
  init_mutex.lock();
  encrypt_->bindBuffer(*p_main_buffer_);
  std::cout << "If you want exit enter: '--stop-client' " << std::endl
            << std::endl;
  init_mutex.unlock();
  // При первом запуске программы, помогает потокам начать работать в нужном
  // порядке(костыль)
...
void Client::startSendingData() {
  ...
  if (!connection_check_) {
    std::cout << "Failed to connect to program two, your data is loss..."
              << std::endl
              << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
  } else {
    auto ret = send(connection_, reinterpret_cast<const char *>(&buffer_),
...
bool Client::checkWantExit() {
  if (want_exit) {
    int stop_code = -9;
    send(connection_, reinterpret_cast<const char *>(&stop_code), sizeof(int),
         MSG_NOSIGNAL);
    closeConnection();
    mtx_->unlock();
    return true;
